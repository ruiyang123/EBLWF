...
np.save("./results_PACS.npy",results)


results = np.load('results_PACS.npy',allow_pickle='TRUE').item()
df = pd.DataFrame()
...
axs[1].set(xlabel=None)
axs[1].xaxis.set_ticks(np.arange(0,6,1))
...
axs[2].set(xlabel=None)
axs[2].xaxis.set_ticks(np.arange(0,7,1))
...
axs[3].set(xlabel=None)
axs[3].xaxis.set_ticks(np.arange(0,6,1))
